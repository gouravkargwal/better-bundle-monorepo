...
export class AnalysisHeuristicService {
  ...
  static async calculateNextAnalysisTime(
    ...
    try {
      ...
      const factors = await this.gatherHeuristicFactors(prisma, shopId, analysisResult);
      ...
      Logger.error("Error calculating heuristic, using fallback", { shopId, error });
   ...
   */
  private static async calculateOrderVolume(prisma: PrismaClient, shopId: string): Promise<number> {
   ...
   */
  private static async calculateRevenueVelocity(prisma: PrismaClient, shopId: string): Promise<number> {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
   ...
   */
  private static async calculateProductChurn(prisma: PrismaClient, shopId: string): Promise<number> {
   ...
   */
  private static async calculateDataChangeRate(prisma: PrismaClient, shopId: string): Promise<number> {
    const lastAnalysis = await prisma.analysisJob.findFirst({
      where: { shopId, status: 'completed' },
      orderBy: { completedAt: 'desc' },
    });
   ...
   */
  private static async calculateUserEngagement(prisma: PrismaClient, shopId: string): Promise<number> {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
