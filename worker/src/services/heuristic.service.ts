...
export class AnalysisHeuristicService {
  ...
  static async calculateNextAnalysisTime(
    ...
    try {
      Logger.info("Calculating heuristic for shop", { shopId });

      // Gather all heuristic factors
      const factors = await this.gatherHeuristicFactors(
        prisma,
        shopId,
        analysisResult
      );
      ...
      const weightedInterval = this.calculateWeightedInterval(
        intervals,
        factors
      );

      // Apply seasonal adjustments
      const seasonalInterval = this.applySeasonalAdjustment(
        weightedInterval,
        factors.seasonality
      );
      ...
      Logger.info("Heuristic calculated successfully", {
        shopId,
        nextAnalysisHours: finalInterval,
        nextAnalysisDays: (finalInterval / 24).toFixed(1),
      });

      return result;
    } catch (error) {
      Logger.error("Error calculating heuristic, using fallback", {
        shopId,
        error,
      });
  ...
  private static async gatherHeuristicFactors(
    ...
    const [
      ...
      this.calculateOrderVolume(prisma, shopId),
      this.calculateRevenueVelocity(prisma, shopId),
      this.calculateProductChurn(prisma, shopId),
      this.calculateBundleEffectiveness(analysisResult),
      this.calculateDataChangeRate(prisma, shopId),
      this.calculateUserEngagement(prisma, shopId),
    ]);
   ...
   */
  private static async calculateOrderVolume(
    prisma: PrismaClient,
    shopId: string
  ): Promise<number> {
   ...
   */
  private static async calculateRevenueVelocity(
    prisma: PrismaClient,
    shopId: string
  ): Promise<number> {
    const thirtyDaysAgo = new Date();
   ...
   */
  private static async calculateProductChurn(
    prisma: PrismaClient,
    shopId: string
  ): Promise<number> {
  ...
  private static calculateBundleEffectiveness(analysisResult: any): number {
    ...
    const avgConfidence =
      analysisResult.bundles.reduce(
        (sum: number, bundle: any) => sum + bundle.confidence,
        0
      ) / analysisResult.bundles.length;

    const avgLift =
      analysisResult.bundles.reduce(
        (sum: number, bundle: any) => sum + bundle.lift,
        0
      ) / analysisResult.bundles.length;
   ...
   */
  private static async calculateDataChangeRate(
    prisma: PrismaClient,
    shopId: string
  ): Promise<number> {
    const lastAnalysis = await prisma.analysisJob.findFirst({
    ...
    const newOrders = await prisma.orderData.count({
      where: {
        shopId,
        orderDate: { gt: lastAnalysisDate || new Date(0) },
      },
    });
  ...
  private static async calculateUserEngagement(
    ...
    const clickEvents = widgetEvents.filter((e) => e.action === "click").length;
    const viewEvents = widgetEvents.filter((e) => e.action === "view").length;
  ...
  private static calculateFactorIntervals(factors: HeuristicFactors) {
    return {
      ...
      effectivenessBased: this.getEffectivenessBasedInterval(
        factors.bundleEffectiveness
      ),
  ...
  private static getChangeBasedInterval(changeRate: number): number {
    if (changeRate > 0.2) return 12; // High change: 12 hours
    if (changeRate > 0.1) return 24; // Medium change: 24 hours
    if (changeRate > 0.05) return 48; // Low change: 48 hours
    if (changeRate > 0.02) return 96; // Minimal change: 4 days
  ...
  private static getActivityBasedInterval(activityLevel: string): number {
    switch (activityLevel) {
      case "high":
        return 12; // High activity: 12 hours
      case "medium":
        return 24; // Medium activity: 24 hours
      case "low":
        return 72; // Low activity: 3 days
      default:
   ...
   */
  private static calculateWeightedInterval(
    intervals: any,
    factors: HeuristicFactors
  ): number {
    const weights = {
      orderBased: 0.25, // 25% weight
      effectivenessBased: 0.2, // 20% weight
      changeBased: 0.2, // 20% weight
      engagementBased: 0.15, // 15% weight
      activityBased: 0.2, // 20% weight
    };

    const weightedSum = Object.entries(intervals).reduce(
      (sum, [key, interval]) => {
        return (
          sum + (interval as number) * weights[key as keyof typeof weights]
        );
      },
      0
    );
  ...
  private static generateReasoning(
    ...
    if (factors.orderVolume > 20) {
      reasoning.push(
        `High order volume (${factors.orderVolume.toFixed(
          1
        )} orders/day) suggests frequent analysis`
      );
    } else if (factors.orderVolume < 5) {
      reasoning.push(
        `Low order volume (${factors.orderVolume.toFixed(
          1
        )} orders/day) allows less frequent analysis`
      );
    }

    if (factors.bundleEffectiveness > 0.7) {
      reasoning.push(
        `High bundle effectiveness (${(
          factors.bundleEffectiveness * 100
        ).toFixed(0)}%) - running analysis more frequently`
      );
    } else if (factors.bundleEffectiveness < 0.3) {
      reasoning.push(
        `Low bundle effectiveness (${(
          factors.bundleEffectiveness * 100
        ).toFixed(0)}%) - less frequent analysis needed`
      );
    }

    if (factors.dataChangeRate > 0.1) {
      reasoning.push(
        `High data change rate (${(factors.dataChangeRate * 100).toFixed(
          0
    ...
    if (factors.seasonality !== 1.0) {
      reasoning.push(
        `Seasonal adjustment applied (${factors.seasonality.toFixed(
          2
        )}x frequency)`
      );
    }
