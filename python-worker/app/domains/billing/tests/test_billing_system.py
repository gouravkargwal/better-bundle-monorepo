...
class TestAttributionEngine(TestBillingSystem):
    ...
    async def test_direct_attribution(self, billing_service, sample_purchase, sample_interactions):
        """Test direct attribution calculation."""
        # Mock recent interactions
        billing_service.prisma.userInteraction.findMany = AsyncMock(return_value=sample_interactions)
        ...
        venus_attribution = next(
            (attr for attr in result.attribution_breakdown if attr.extension == "venus"),
            None
        )
    ...
    async def test_cross_extension_attribution(self, billing_service, sample_purchase):
        ...
        billing_service.prisma.userInteraction.findMany = AsyncMock(return_value=multi_extension_interactions)
        ...
        venus_attr = next((attr for attr in result.attribution_breakdown if attr.extension == "venus"), None)
        atlas_attr = next((attr for attr in result.attribution_breakdown if attr.extension == "atlas"), None)
...
class TestFraudDetection(TestBillingSystem):
    ...
    async def test_excessive_interactions_detection(self, billing_service, sample_shop_id):
        ...
        billing_service.prisma.userInteraction.findMany = AsyncMock(return_value=excessive_interactions)
    ...
    async def test_rapid_fire_clicks_detection(self, billing_service, sample_shop_id):
        ...
        billing_service.prisma.userInteraction.findMany = AsyncMock(return_value=rapid_clicks)
        billing_service.prisma.purchaseAttribution.findMany = AsyncMock(return_value=[])
    ...
    async def test_bot_traffic_detection(self, billing_service, sample_shop_id):
        ...
        billing_service.prisma.userInteraction.findMany = AsyncMock(return_value=bot_interactions)
        billing_service.prisma.purchaseAttribution.findMany = AsyncMock(return_value=[])
...
class TestBillingCalculator(TestBillingSystem):
    ...
    async def test_performance_tier_calculation(self, billing_service, sample_shop_id, sample_billing_period):
        ...
        billing_plan.configuration = {
            "performance_tiers": [
                ...
                {"name": "Tier 2", "min_revenue": 5000, "max_revenue": 25000, "rate": 0.025},
                {"name": "Tier 3", "min_revenue": 25000, "max_revenue": None, "rate": 0.02}
            ]
        }
        
        billing_service.billing_repository.get_billing_plan = AsyncMock(return_value=billing_plan)
        ...
        for case in test_cases:
            ...
            assert result["calculation"]["base_fee"] == case["revenue"] * case["expected_rate"]
            assert result["plan_id"] == "plan-123"
    
    async def test_fraud_adjustment(self, billing_service, sample_shop_id, sample_billing_period):
...
class TestNotificationService(TestBillingSystem):
    ...
    async def test_invoice_notification(self, mock_client, billing_service, sample_shop_id):
        ...
        # Mock shop data
        billing_service.prisma.shop.find_unique = AsyncMock(return_value=Mock(email="test@example.com"))
    ...
    @patch('httpx.AsyncClient')
    async def test_fraud_alert_notification(self, mock_client, billing_service, sample_shop_id):
...
class TestEndToEndBilling(TestBillingSystem):
    ...
    async def test_complete_monthly_billing_workflow(self, billing_service, sample_shop_id):
        ...
        billing_service.billing_repository.get_billing_plan = AsyncMock(return_value=billing_plan)
        billing_service.billing_repository.get_shop_attribution_data = AsyncMock(return_value={
            "attributed_revenue": 10000,
        ...
        billing_service.billing_repository.create_billing_metrics = AsyncMock()
        ...
        billing_service.fraud_detection_service.analyze_shop_fraud_risk = AsyncMock(return_value=fraud_result)
    ...
    async def test_high_fraud_risk_billing_workflow(self, billing_service, sample_shop_id):
    ...
    async def test_shopify_integration_workflow(self, billing_service, sample_shop_id):
        ...
        billing_service.shopify_billing_service.create_monthly_billing_charge = AsyncMock(return_value=shopify_charge)
        ...
        result = await billing_service.process_monthly_billing_with_shopify(sample_shop_id)
...
class TestBillingRepository(TestBillingSystem):
    ...
    async def test_billing_plan_creation(self, billing_service, sample_shop_id):
        ...
        billing_service.billing_repository.create_billing_plan = AsyncMock(return_value=created_plan)
        
        # Create billing plan
        result = await billing_service.create_default_billing_plan(sample_shop_id, "test-shop.myshopify.com")
    ...
    async def test_billing_metrics_creation(self, billing_service, sample_shop_id, sample_billing_period):
        """Test billing metrics creation."""
        # Mock metrics creation
        created_metrics = Mock()
        created_metrics.id = "metrics-123"
        created_metrics.totalRevenue = Decimal("10000")
        created_metrics.attributedRevenue = Decimal("10000")
        created_metrics.finalFee = Decimal("300")
        
        billing_service.billing_repository.create_billing_metrics = AsyncMock(return_value=created_metrics)
        ...
        result = await billing_service.billing_repository.create_billing_metrics(
            sample_shop_id, "plan-123", sample_billing_period, metrics_data
        )
        
        # Verify metrics creation
        assert result.id == "metrics-123"
        assert result.totalRevenue == Decimal("10000")
        assert result.finalFee == Decimal("300")
...
async def run_integration_tests():
    ...
    await attribution_tests.test_direct_attribution(billing_service, test_suite.sample_purchase(), test_suite.sample_interactions())
    await attribution_tests.test_cross_extension_attribution(billing_service, test_suite.sample_purchase())
    ...
    await fraud_tests.test_excessive_interactions_detection(billing_service, test_suite.sample_shop_id())
    await fraud_tests.test_rapid_fire_clicks_detection(billing_service, test_suite.sample_shop_id())
    await fraud_tests.test_bot_traffic_detection(billing_service, test_suite.sample_shop_id())
    ...
    await calculator_tests.test_performance_tier_calculation(billing_service, test_suite.sample_shop_id(), test_suite.sample_billing_period())
    await calculator_tests.test_fraud_adjustment(billing_service, test_suite.sample_shop_id(), test_suite.sample_billing_period())
    print("✅ Billing Calculator tests passed")
    
    # Run notification tests
    print("📧 Testing Notification Service...")
    notification_tests = TestNotificationService()
    await notification_tests.test_invoice_notification(billing_service, test_suite.sample_shop_id())
    await notification_tests.test_fraud_alert_notification(billing_service, test_suite.sample_shop_id())
    print("✅ Notification Service tests passed")
    
    # Run end-to-end tests
    print("🔄 Testing End-to-End Workflows...")
    e2e_tests = TestEndToEndBilling()
    await e2e_tests.test_complete_monthly_billing_workflow(billing_service, test_suite.sample_shop_id())
    await e2e_tests.test_high_fraud_risk_billing_workflow(billing_service, test_suite.sample_shop_id())
    await e2e_tests.test_shopify_integration_workflow(billing_service, test_suite.sample_shop_id())
    print("✅ End-to-End Workflow tests passed")
    
    # Run repository tests
    print("🗄️ Testing Billing Repository...")
    repository_tests = TestBillingRepository()
    await repository_tests.test_billing_plan_creation(billing_service, test_suite.sample_shop_id())
    await repository_tests.test_billing_metrics_creation(billing_service, test_suite.sample_shop_id(), test_suite.sample_billing_period())
