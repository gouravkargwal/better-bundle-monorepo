"""
Adapter for product_removed_from_cart events
"""
...
class CartRemoveAdapter(BaseInteractionEventAdapter):
    """Adapter for product_removed_from_cart events"""

    def __init__(self):
        super().__init__("product_removed_from_cart")

    def extract_product_id(self, event: Dict[str, Any]) -> Optional[str]:
        """Extract product ID from product_removed_from_cart event"""
        try:
    ...
    def extract_metadata(self, event: Dict[str, Any]) -> Dict[str, Any]:
        """Extract relevant metadata from product_removed_from_cart event"""
        try:
            metadata = event.get("metadata", {})
            data = metadata.get("data", {})
            cart_line = data.get("cartLine", {})
            ...
            return {
                ...
                "removal_reason": "user_action",  # Could be enhanced with actual reason
