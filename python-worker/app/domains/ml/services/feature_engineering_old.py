...
class FeatureEngineeringService(IFeatureEngineeringService):
    ...
    async def normalize_features(
        ...
        try:
            ...
            # to calculate statistics across the entire dataset for the shop
    ...
    async def _get_dataset_statistics(
        ...
        try:
            ...
            # For now, return empty stats which will result in identity normalization
            # TODO: Implement proper database queries to get mean, std, min, max, median, iqr
            # for each feature across the entire dataset
                ...
                if isinstance(feature_value, str) and feature_value is not None:
                    if encoding_method == "one_hot":
                        ...
                        encoded_features[feature_name] = (
                            hash(feature_value) % self.hashing_modulus
                        )
                    else:
    ...
    def _compute_basic_product_features(
        ...
        return {
            ...
            "vendor_encoded": hash(product.vendor) % self.hashing_modulus,
            "product_type_encoded": hash(product.product_type) % self.hashing_modulus,
    ...
    def _compute_shop_context_features(
        ...
        return {
            "shop_plan_encoded": hash(shop.plan_name or "") % self.hashing_modulus,
            "shop_currency_encoded": hash(shop.currency or "") % self.hashing_modulus,
            "shop_country_encoded": hash(shop.country or "") % self.hashing_modulus,
    ...
    def _compute_address_features(self, customer: ShopifyCustomer) -> Dict[str, Any]:
        ...
        return {
            "has_address": 1,
            "country_encoded": hash(address.country or "") % self.hashing_modulus,
            "province_encoded": hash(address.province or "") % self.hashing_modulus,
            "city_encoded": hash(address.city or "") % self.hashing_modulus,
    ...
    def _compute_basic_order_features(self, order: ShopifyOrder) -> Dict[str, Any]:
        ...
        return {
            ...
            "currency_encoded": hash(order.currency or "") % self.hashing_modulus,
            "financial_status_encoded": hash(order.financial_status or "")
            % self.hashing_modulus,
            "fulfillment_status_encoded": hash(order.fulfillment_status or "")
            % self.hashing_modulus,
    ...
    def _compute_basic_collection_features(
        ...
        return {
            ...
            "sort_order_encoded": hash(collection.sort_order or "")
            % self.hashing_modulus,
        }
    ...
    def _compute_basic_shop_features(self, shop: ShopifyShop) -> Dict[str, Any]:
        ...
        return {
            ...
            "plan_encoded": hash(shop.plan_name or "") % self.hashing_modulus,
            "currency_encoded": hash(shop.currency or "") % self.hashing_modulus,
            "country_encoded": hash(shop.country or "") % self.hashing_modulus,
            "timezone_encoded": hash(shop.iana_timezone or "") % self.hashing_modulus,
    ...
    async def _compute_and_save_product_features_batch(
        ...
        try:
            ...
            for product in products:
                ...
                batch_data.append(
                    (
                        ...
                        / self.normalization_divisors["variant_count"],
                        features.get("image_count", 0)
                        / self.normalization_divisors["image_count"],
                        features.get("tag_diversity", 0)
                        / self.normalization_divisors["tag_diversity"],
            ...
            # Use bulk upsert with raw SQL to avoid N+1 problem
            saved_count = await self._bulk_upsert_product_features(db, batch_data)
    ...
    async def _compute_and_save_customer_features_batch(
        ...
        try:
            ...
            # Fetch all orders for these customers in one query
            orders_query = """
                SELECT * FROM "ShopifyOrder" 
                WHERE "shopId" = $1 AND "customerId" = ANY($2)
            """
            orders_result = await db.fetch_all(orders_query, shop_id, customer_ids)
            orders = [ShopifyOrder(**order) for order in orders_result]
            ...
            # Fetch all events for these customers in one query
            events_query = """
                SELECT * FROM "ShopifyCustomerEvent" 
                WHERE "shopId" = $1 AND "customerId" = ANY($2)
            """
            events_result = await db.fetch_all(events_query, shop_id, customer_ids)
            events = [ShopifyCustomerEvent(**event) for event in events_result]

            # Group events by customer ID for efficient lookup
            events_by_customer = {}
            for event in events:
                if event.customer_id not in events_by_customer:
                    events_by_customer[event.customer_id] = []
            ...
            for customer in customers:
                # Get customer's orders and events from the batch-fetched data
                customer_orders = orders_by_customer.get(customer.id, [])
                customer_events = events_by_customer.get(customer.id, [])

                # Compute features with actual data
            ...
            # Use bulk upsert to avoid N+1 problem
            saved_count = await self._bulk_upsert_customer_features(
                db, user_features_batch, behavior_features_batch
            )
    ...
    async def _compute_and_save_collection_features_batch(
        ...
        try:
            ...
            # Use bulk upsert to avoid N+1 problem
            saved_count = await self._bulk_upsert_collection_features(db, batch_data)
            return {
    ...
    async def _compute_and_save_interaction_features_batch(
        ...
        try:
            ...
            # Use bulk upsert to avoid N+1 problem
            saved_count = await self._bulk_upsert_interaction_features(db, batch_data)
            return {
                "saved_count": saved_count,
    ...
    # Bulk upsert methods to fix N+1 write problem
    async def _bulk_upsert_product_features(self, db, batch_data: List[tuple]) -> int:
        ...
        try:
            ...
            # This is safer than trying to build a complex multi-row parameterized query
    ...
    async def _bulk_upsert_customer_features(
        self, db, user_features_batch: List[tuple], behavior_features_batch: List[tuple]
    ) -> int:
    ...
    async def _bulk_upsert_collection_features(
        self, db, batch_data: List[tuple]
    ) -> int:
    ...
    async def _bulk_upsert_interaction_features(
        self, db, batch_data: List[tuple]
    ) -> int:
        ...
        try:
            ...
            for product_data in products:
                ...
                await db.execute_raw(
                    ...
                    product.product_type,
                    features.get("variant_count", 0)
                    / self.normalization_divisors["variant_count"],  # Normalize
                    features.get("image_count", 0)
                    / self.normalization_divisors["image_count"],  # Normalize
                    features.get("tag_diversity", 0)
                    / self.normalization_divisors["tag_diversity"],  # Normalize
                    1 if product.product_type else 0,
                    1 if product.vendor else 0,
                    now_utc(),
                )
