...
class AnalysisHeuristicService:
    ...
    async def calculate_next_analysis_time(
        ...
        try:
            ...
            seasonal_interval = self._apply_seasonal_adjustment(weighted_interval, factors.seasonality)
    ...
    async def _gather_heuristic_factors(
        ...
        shop_activity_level = self._determine_activity_level(order_volume, revenue_velocity)
    ...
    async def _calculate_order_volume(self, shop_id: str) -> float:
        ...
        try:
            ...
            logger.error("Error calculating order volume", shop_id=shop_id, error=str(e))
            return 0.0
    ...
    async def _calculate_revenue_velocity(self, shop_id: str) -> float:
        ...
        try:
            ...
            logger.error("Error calculating revenue velocity", shop_id=shop_id, error=str(e))
            return 0.0
    ...
    async def _calculate_product_churn(self, shop_id: str) -> float:
        ...
        try:
            ...
            logger.error("Error calculating product churn", shop_id=shop_id, error=str(e))
            return 0.0
    
    def _calculate_bundle_effectiveness(self, analysis_result: Optional[Dict[str, Any]]) -> float:
    ...
    async def _calculate_data_change_rate(self, shop_id: str) -> float:
        ...
        try:
            ...
            logger.error("Error calculating data change rate", shop_id=shop_id, error=str(e))
            return 0.0
    ...
    async def _calculate_user_engagement(self, shop_id: str) -> float:
        ...
        try:
            ...
            logger.error("Error calculating user engagement", shop_id=shop_id, error=str(e))
            return 0.0
    ...
    def _determine_activity_level(self, order_volume: float, revenue_velocity: float) -> str:
    ...
    def _calculate_factor_intervals(self, factors: HeuristicFactors) -> Dict[str, float]:
    ...
    def _calculate_weighted_interval(self, intervals: Dict[str, float], factors: HeuristicFactors) -> float:
        ...
        weighted_sum = sum(intervals[key] * weights[key] for key in weights if key in intervals)
        return weighted_sum
    ...
    def _generate_reasoning(
        ...
        if factors.order_volume > 20:
            reasoning.append(f"High order volume ({factors.order_volume:.1f} orders/day) suggests frequent analysis")
        elif factors.order_volume < 5:
            reasoning.append(f"Low order volume ({factors.order_volume:.1f} orders/day) allows less frequent analysis")
        
        if factors.bundle_effectiveness > 0.7:
            reasoning.append(f"High bundle effectiveness ({(factors.bundle_effectiveness * 100):.0f}%) - running analysis more frequently")
        elif factors.bundle_effectiveness < 0.3:
            reasoning.append(f"Low bundle effectiveness ({(factors.bundle_effectiveness * 100):.0f}%) - less frequent analysis needed")
        
        if factors.data_change_rate > 0.1:
            reasoning.append(f"High data change rate ({(factors.data_change_rate * 100):.0f}%) requires frequent updates")
        
        if factors.seasonality != 1.0:
            reasoning.append(f"Seasonal adjustment applied ({factors.seasonality:.2f}x frequency)")
    ...
    async def store_heuristic_decision(
        ...
        try:
            ...
            logger.error("Error storing heuristic decision", shop_id=shop_id, error=str(e))
    ...
    async def schedule_next_analysis(self, shop_id: str, analysis_result: Optional[Dict[str, Any]] = None):
        """Schedule next analysis for a shop"""
        try:
            # Calculate next analysis time
            heuristic_result = await self.calculate_next_analysis_time(shop_id, analysis_result)
            
            # Calculate next scheduled time
            next_scheduled_time = datetime.now() + timedelta(hours=heuristic_result.next_analysis_hours)
            ...
            # Store heuristic decision for learning
            await self.store_heuristic_decision(shop_id, heuristic_result, analysis_result)
