...
class ScheduledAnalysis:
    ...
    def can_retry(self) -> bool:
        ...
        return self.retry_count < self.max_retries and self.status in [ScheduleStatus.FAILED, ScheduleStatus.OVERDUE]
...
class SchedulerService:
            ...
            default_config = {
                'cron_schedules': [
                    ...
                    },
                    {
                        ...
                        'cron_expression': '0 2 * * *',
                        'analysis_types': ['performance_analytics', 'customer_analytics'],
                        'priority': 'medium',
                        'enabled': True,
                        'timezone': 'UTC'
                    },
        ...
        try:
            ...
            for schedule_name, cron_schedule in self.cron_schedules.items():
                ...
                if self._should_run_cron_schedule(cron_schedule, current_time):
                    ...
                    await self._create_scheduled_analysis_from_cron(cron_schedule, current_time)
                    
        except Exception as e:
            self.logger.error(f"Failed to process cron schedules: {e}")
    
    def _should_run_cron_schedule(self, cron_schedule: CronSchedule, current_time: datetime) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to check cron schedule {cron_schedule.name}: {e}")
            return False
    
    async def _create_scheduled_analysis_from_cron(self, cron_schedule: CronSchedule, current_time: datetime):
    ...
    async def _process_scheduled_analyses(self):
        ...
        try:
            ...
            for analysis_id, analysis in self.scheduled_analyses.items():
                if (analysis.status == ScheduleStatus.PENDING and 
                    analysis.scheduled_time <= current_time and
        ...
        try:
            ...
            for analysis in overdue_analyses:
                ...
                self.logger.warning(
                    ...
                    overdue_minutes=int((current_time - analysis.scheduled_time).total_seconds() / 60)
                )
    ...
    async def _execute_scheduled_analysis(self, analysis: ScheduledAnalysis):
        ...
        try:
            ...
            self.logger.error(f"Failed to execute scheduled analysis: {e}", analysis_id=analysis.id)
    ...
    async def _execute_for_single_shop(self, analysis: ScheduledAnalysis, shop_id: Optional[str] = None):
        ...
        try:
            ...
            for analysis_type in analysis.analysis_types:
                ...
                if analysis_type in ["ml_model_retraining", "ml_training"]:
                    ...
                    await streams_manager.publish_event(DATA_JOB_STREAM, job_data)
                ...
                self.logger.info(
                    ...
                    shop_id=target_shop_id,
                    analysis_type=analysis_type,
                    stream=ML_TRAINING_STREAM if analysis_type in ["ml_model_retraining", "ml_training"] else DATA_JOB_STREAM
                )
            
        except Exception as e:
            self.logger.error(f"Failed to execute for single shop: {e}", shop_id=target_shop_id)
            raise
    ...
    async def cancel_analysis(self, analysis_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to cancel analysis: {e}", analysis_id=analysis_id)
            return False
    ...
    async def update_cron_schedule(
        ...
        try:
            ...
            self.logger.info(f"Updated cron schedule", schedule_name=schedule_name, updates=updates)
            
            return True
            
        except Exception as e:
            self.logger.error(f"Failed to update cron schedule: {e}", schedule_name=schedule_name)
            return False
...
scheduler_service = SchedulerService(heuristic_service=None)  # Will be set when service is initialized
