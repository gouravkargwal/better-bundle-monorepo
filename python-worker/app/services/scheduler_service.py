...
class SchedulerService:
        ...
        try:
            ...
            for shop_config in shops_due:
                try:
                    ...
                    message_id = await redis_streams_manager.publish_data_job_event({
                        "jobId": job_id,
                        "shopId": shop.id,
                        ...
                        "scheduledTime": shop_config.nextScheduledAnalysis.isoformat() if shop_config.nextScheduledAnalysis else None
                    })
                    
                    logger.info(f"✅ Scheduled analysis triggered for {shop.shopDomain}: {message_id}")
                    ...
                    logger.error(f"❌ Failed to trigger analysis for shop {shop_config.shop.shopDomain}: {str(e)}")
                    failed_count += 1
                    results.append({
                        "shop_id": shop_config.shop.id if shop_config.shop else None,
                        "shop_domain": shop_config.shop.shopDomain if shop_config.shop else "unknown",
                        "error": str(e),
                        "status": "failed"
                    })
            
            logger.info(f"✅ Scheduler completed: {processed_count} processed, {failed_count} failed")
    ...
    async def trigger_manual_analysis(self, shop_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not shop:
                return {
                    "success": False,
                    "error": f"Shop not found: {shop_id}"
                }
            ...
            logger.info(f"✅ Manual analysis triggered for {shop.shopDomain}: {message_id}")
            ...
            logger.error(f"❌ Error triggering manual analysis for shop {shop_id}: {str(e)}")
            return {
                "success": False,
                "error": str(e)
            }
    ...
    async def get_scheduler_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            seven_days_from_now = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
                ...
                "upcoming_analyses": [
                    {
                        ...
                        "scheduled_time": analysis.nextScheduledAnalysis.isoformat() if analysis.nextScheduledAnalysis else None
                    }
    ...
    async def update_analysis_schedule(self, shop_id: str, analysis_result: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.error(f"❌ Failed to update analysis schedule for shop {shop_id}: {result.get('error')}")
                return result
                
        except Exception as e:
            logger.error(f"❌ Error updating analysis schedule for shop {shop_id}: {str(e)}")
            return {
                "success": False,
                "error": str(e)
            }
