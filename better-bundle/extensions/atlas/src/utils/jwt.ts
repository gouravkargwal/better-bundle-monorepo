...
const getStoredToken = async (
  ...
  try {
    ...
    const shopDomain = await sessionStorage.getItem(SHOP_DOMAIN_KEY);
    ...
    return {
      token,
      expiresIn: parseInt(
        (await sessionStorage.getItem(TOKEN_EXPIRY_KEY)) || "0",
      ),
      shopDomain,
      shopStatus: "unknown",
      permissions: [],
...
const storeToken = async (
  ...
  try {
    await sessionStorage.setItem(TOKEN_KEY, tokenInfo.token);
    const expiryTimestamp = Math.floor(Date.now() / 1000) + tokenInfo.expiresIn;
    await sessionStorage.setItem(TOKEN_EXPIRY_KEY, expiryTimestamp.toString());
    await sessionStorage.setItem(SHOP_DOMAIN_KEY, tokenInfo.shopDomain);
  } catch (error) {
...
const fetchNewTokenPair = async (
  ...
  // Store refresh token
  await sessionStorage.setItem(REFRESH_TOKEN_KEY, data.refresh_token);
...
const refreshToken = async (
  ...
  refreshPromise = fetchNewTokenPair(sessionStorage, shopDomain, customerId);

  try {
    const tokenInfo = await refreshPromise;
    await storeToken(sessionStorage, tokenInfo);
  ...
  refreshAccessTokenPromise = (async (): Promise<string | null> => {
    try {
      ...
      if (!storedRefreshToken) {
        logger.warn(
          "Atlas pixel: No refresh token found, generating new token pair",
        );
        // Fallback to generating new token pair
        return await refreshToken(sessionStorage, shopDomain, customerId);
      }
      ...
      if (response.ok) {
        ...
        logger.info("Atlas pixel: Access token refreshed successfully");
        return tokenInfo.token;
      } else {
        // Refresh token expired or invalid - fallback to new token pair
        logger.warn(
          "Atlas pixel: Refresh token failed, generating new token pair",
        );
        await sessionStorage.removeItem(REFRESH_TOKEN_KEY);
        return await refreshToken(sessionStorage, shopDomain, customerId);
      }
    } catch (error) {
      logger.error({ error }, "Atlas pixel: Failed to refresh access token");
      // Fallback to generating new token pair
      return await refreshToken(sessionStorage, shopDomain, customerId);
    }
  })();
...
const getValidToken = async (
  ...
  const storedToken = await getStoredToken(sessionStorage);
...
export const makeAuthenticatedRequest = async (
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      let token = await getValidToken(sessionStorage, shopDomain, customerId);
      ...
      if (response.status === 401 && attempt === 0) {
        logger.warn("Atlas pixel: Received 401, attempting token refresh");
        ...
        if (newToken) {
          ...
          logger.error("Atlas pixel: Token refresh failed");
      ...
      if (response.status >= 500 && response.status < 600) {
        if (attempt < MAX_RETRIES - 1) {
          ...
          logger.warn(
            `Atlas pixel: Server error ${response.status}, retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`,
          );
      ...
      if (response.status === 429 && attempt < MAX_RETRIES - 1) {
        ...
        logger.warn(
          `Atlas pixel: Rate limited (429), retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
        continue; // Retry
      }
      ...
      if (attempt < MAX_RETRIES - 1) {
        ...
        logger.warn(
          `Atlas pixel: Network error (${error?.message || "unknown"}), retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
        continue; // Retry
      }
