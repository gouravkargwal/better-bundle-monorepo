 ...
 */
const getStoredToken = async (storage) => {
  try {
    ...
    logger.error({ error }, "Mercury pixel: Failed to get stored token");
    return null;
  }
};
...
const storeToken = async (storage, tokenInfo) => {
  try {
    ...
    logger.error({ error }, "Mercury pixel: Failed to store token");
  }
};
 ...
 */
const fetchNewTokenPair = async (storage, shopDomain, customerId = null) => {
  ...
  if (!response.ok) {
    logger.error({ response }, "Mercury pixel: Failed to fetch new token");
 ...
 */
const refreshToken = async (storage, shopDomain, customerId = null) => {
  if (refreshPromise) {
    try {
  ...
  try {
    ...
    logger.error({ error }, "Mercury pixel: Failed to refresh token");
    return null;
  } finally {
    refreshPromise = null;
  }
 ...
 */
const refreshAccessToken = async (storage, shopDomain, customerId = null) => {
  ...
  refreshAccessTokenPromise = (async () => {
    try {
      ...
      if (!storedRefreshToken) {
        logger.warn(
          "Mercury pixel: No refresh token found, generating new token pair",
        );
      ...
      if (response.ok) {
        ...
        logger.info("Mercury pixel: Access token refreshed successfully");
        return tokenInfo.token;
      } else {
        // Refresh token expired or invalid - fallback to new token pair
        logger.warn(
          "Mercury pixel: Refresh token failed, generating new token pair",
        );
        await storage.remove(REFRESH_TOKEN_KEY);
        return await refreshToken(storage, shopDomain, customerId);
      }
    } catch (error) {
      logger.error({ error }, "Mercury pixel: Failed to refresh access token");
      // Fallback to generating new token pair
      return await refreshToken(storage, shopDomain, customerId);
    }
  })();
 ...
 */
const getValidToken = async (storage, shopDomain, customerId = null) => {
  ...
  try {
    ...
    logger.error({ error }, "Mercury pixel: Failed to get valid token");
    return null;
  }
};
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      if (response.status === 401 && attempt === 0) {
        logger.warn("Mercury pixel: Received 401, attempting token refresh");
      ...
      if (response.status >= 500 && response.status < 600) {
        if (attempt < MAX_RETRIES - 1) {
          ...
          logger.warn(
            `Mercury pixel: Server error ${response.status}, retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`,
          );
      ...
      if (response.status === 429 && attempt < MAX_RETRIES - 1) {
        ...
        logger.warn(
          `Mercury pixel: Rate limited (429), retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
        continue; // Retry
      }
      ...
      if (attempt < MAX_RETRIES - 1) {
        ...
        logger.warn(
          `Mercury pixel: Network error (${error?.message || "unknown"}), retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
        continue; // Retry
      }
