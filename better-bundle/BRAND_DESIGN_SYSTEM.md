...
**Why this replaces the decorative circles:** The existing components ([`OnboardingHero`](better-bundle/app/features/onboarding/components/OnboardingHero.tsx:43), [`FeatureCard`](better-bundle/app/features/onboarding/components/FeatureCard.tsx:108), [`Benefits`](better-bundle/app/features/onboarding/components/Benefits.tsx:57)) each have `radial-gradient` circles in their corners. These are decorative calories — they add texture but no information. The Flat Line is both decorative AND meaningful: it encodes the pricing story on every container it appears on.
...
--bb-primitive-space-3: 12px;
/* ... (full scale) */

/* Typography Primitives */
--bb-primitive-font-inter:
...
--bb-color-brand-gradient: linear-gradient(
  135deg,
...
/* === PRICING / SUCCESS === */
--bb-color-success: var(
  --bb-primitive-green-600
); /* #059669 — AA compliant on white */
--bb-color-success-light: var(
  --bb-primitive-green-500
); /* #10B981 — icons, large elements */
--bb-color-success-gradient: linear-gradient(
  135deg,
...
| **Error**     | Full-width [`Banner`](better-bundle/app/features/onboarding/components/OnboardingPage.tsx:40) below hero with specific error message + retry. Hero stays visible. |
...
- Inline `onMouseEnter`/`onMouseLeave` in [`FeatureCard.tsx`](better-bundle/app/features/onboarding/components/FeatureCard.tsx:205-213) causes layout thrashing (no `will-change` transform)
...
**Current:** Badge "Complete Solution" + H2 "Everything You Need to Succeed" in [`Benefits.tsx`](better-bundle/app/features/onboarding/components/Benefits.tsx:99-114) — generic.
...
**Current [`OverviewHero.tsx`](better-bundle/app/features/onboarding/components/OverviewHero.tsx:41-61):** Same gradient as OnboardingHero (good consistency). "Better Bundle" as heading (appropriate for overview — different context). Tagline "AI-powered recommendations that pay for themselves" is strong.
...
**Current [`ValueCommunicationBanner.tsx`](better-bundle/app/features/overview/components/ValueCommunicationBanner.tsx:31-93):** Color-coded by status (amber/active, green/success, red/suspended). The ROI math is the strongest part: "You paid only $X for $Y in revenue."
...
Standardize the inline badge styles currently duplicated across components ([`OnboardingHero.tsx:46-58`](better-bundle/app/features/onboarding/components/OnboardingHero.tsx:46), [`FeatureCard.tsx:281-304`](better-bundle/app/features/onboarding/components/FeatureCard.tsx:281), [`Benefits.tsx:88-101`](better-bundle/app/features/onboarding/components/Benefits.tsx:88)):
...
| ---------------------------- | -------------------------------------------------------- | ---------------------------------------------- | ----------------------------- |
| **OnboardingHero**           | ✅ Button spinner + text swap (`OnboardingHero.tsx:254`) | ✅ Banner below hero (`OnboardingPage.tsx:40`) | N/A (defaults always present) |
...
- Error boundaries at the route level ([`app.tsx:123`](better-bundle/app/routes/app.tsx:123)) — currently uses Shopify's `boundary.error` which is generic
...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| **Lead with the pricing model**  | [`"Welcome to BetterBundle!"`](better-bundle/app/features/onboarding/components/OnboardingHero.tsx:75)                                                | `"Flat-Rate AI Recommendations — $29/month"`                                                                         |
...
| **Be specific, not clever**      | [`"New AI-Powered Solution"`](better-bundle/app/features/onboarding/components/OnboardingHero.tsx:56) (vague)                                         | `"1,500+ Shopify stores already using BetterBundle"` (specific)                                                      |
| **Active voice**                 | `"Web pixel will be activated"`                                                                                                                       | `"We activate your recommendations"`                                                                                 |
| **Error: explain + guide**       | [`"Please try again or contact support if the issue persists."`](better-bundle/app/features/onboarding/components/OnboardingPage.tsx:95-97) (generic) | `"We couldn't start your trial. Your billing may need confirmation — check your Shopify admin or retry."` (specific) |
...
| Before ([`FeatureCard.tsx:28-91`](better-bundle/app/features/onboarding/components/FeatureCard.tsx:28)) | After                           | Rationale                                   |
| ------------------------------------------------------------------------------------------------------- | ------------------------------- | ------------------------------------------- |
...
| Context                  | Before ([`OnboardingPage.tsx:95`](better-bundle/app/features/onboarding/components/OnboardingPage.tsx:95)) | After                                                                                                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
The `radial-gradient` circles in the corners of [`OnboardingHero`](better-bundle/app/features/onboarding/components/OnboardingHero.tsx:43), [`FeatureCard`](better-bundle/app/features/onboarding/components/FeatureCard.tsx:108), and [`Benefits`](better-bundle/app/features/onboarding/components/Benefits.tsx:57) are pure decoration — they add no information and repeat across every component. The Flat Line replaces them with a device that is BOTH decorative AND meaningful (it encodes the pricing story). Per the design principle: _"Spend boldness in one place."_ The Flat Line is that place.

### Phase 3: Copy & CRO Improvements (next sprint)

| #   | Task                                                          | Files Affected       | Effort |
| --- | ------------------------------------------------------------- | -------------------- | ------ |
| 12  | Rewrite OnboardingHero H1 + subheadline + CTA per Section 4.2 | `OnboardingHero.tsx` | Small  |
| 13  | Add secondary CTA in OnboardingHero                           | `OnboardingHero.tsx` | Small  |
| 14  | Rewrite FeatureCard titles per Section 4.3                    | `FeatureCard.tsx`    | Small  |
| 15  | Add revenue-generating micro-copy line to hero                | `OnboardingHero.tsx` | Tiny   |
| 16  | Standardize error messages per Section 4.4                    | `OnboardingPage.tsx` | Small  |

### Phase 4: Interactive State Standardization (next sprint)

| #   | Task                                                                        | Files Affected     | Effort |
| --- | --------------------------------------------------------------------------- | ------------------ | ------ |
| 17  | Remove inline `onMouseEnter`/`onMouseLeave` from FeatureCard → CSS `:hover` | `FeatureCard.tsx`  | Small  |
| 18  | Add `prefers-reduced-motion` query to all transitions                       | Global CSS         | Small  |
| 19  | Add `:focus-visible` outlines to all interactive elements                   | All components     | Medium |
| 20  | Add skeleton loading state to OverviewHero                                  | `OverviewHero.tsx` | Small  |

### Phase 5: Badge & Layout Standardization (future sprint)

| #   | Task                                                   | Files Affected   | Effort |
| --- | ------------------------------------------------------ | ---------------- | ------ |
| 21  | Create shared Badge variant CSS using Section 3.1.6    | New `_badge.css` | Small  |
| 22  | Replace all inline badge styles with variant classes   | All components   | Medium |
| 23  | Apply `--bb-layout-*` tokens to all section containers | All components   | Medium |
...
- ❌ Install Tailwind CSS or any new CSS framework — Polaris + CSS variables covers everything
- ❌ Add styled-components, CSS modules, or any CSS-in-JS library — the team already ships inline styles; CSS variables are the least-risky migration path
- ❌ Create a separate display typeface — Inter at extreme size/weight weights provides sufficient differentiation within the Polaris ecosystem
- ❌ Add decorative animations or scroll-triggered reveals — motion is reserved for functional state changes only
...
Shopify merchants see Inter everywhere in their admin. Introducing a foreign display face would create visual dissonance. Instead, the design system differentiates through:
...
- Compatible with Polaris (variables cascade alongside Polaris tokens)
- Zero build step (no Tailwind JIT, no PostCSS plugins)
- Incrementally adoptable (components switch one at a time)
- Themeable (dark mode = change variable values, not components)
...
The `radial-gradient` circles in the corners of hero containers are pure decoration — they add no information and repeat across every component. The Flat Line replaces them with a device that is BOTH decorative AND meaningful (it encodes the pricing story). Per the design principle: _"Spend boldness in one place."_ The Flat Line is that place.
