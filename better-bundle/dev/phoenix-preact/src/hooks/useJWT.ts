...
export function useJWT(shopDomain?: string, customerId?: string) {
  ...
  const getStoredToken = async (): Promise<TokenInfo | null> => {
    try {
      ...
      const shopDomain = localStorage.getItem(SHOP_DOMAIN_KEY);

      if (!token || !shopDomain) return null;

      return {
        token,
        expiresIn: parseInt(localStorage.getItem(TOKEN_EXPIRY_KEY) || "0"),
        shopDomain,
        shopStatus: "unknown",
        permissions: [],
      };
  ...
  const storeToken = async (tokenInfo: TokenInfo): Promise<void> => {
    try {
      ...
      localStorage.setItem(SHOP_DOMAIN_KEY, tokenInfo.shopDomain);
    } catch (error) {
      // Silently fail
    }
  };
