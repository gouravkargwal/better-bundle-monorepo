...
def ts(sql_select, stream, extra=""):
    ...
    return (
        f"SELECT histogram(_timestamp) as x_axis_1, {sql_select} as y_axis_1"
...
health = dashboard(
    ...
    [
        panel(
            1,
            "Request Rate",
            "Requests per time bucket, all services",
            "http_server_request_duration_count",
            ts("sum(value)", "http_server_request_duration_count"),
            "Requests",
            x=0,
            y=0,
            w=12,
        ...
        panel(
            3,
            "Average Latency",
            "sum/count of the duration histogram, in seconds",
        ...
        panel(
            ...
            ts("max(value)", "http_server_active_requests"),
            "Active",
            x=12,
            y=9,
            w=12,
        ...
        panel(
            ...
            "Where the traffic is landing",
            "http_server_request_duration_count",
            "SELECT http_response_status_code as x_axis_1, sum(value) as y_axis_1 "
            'FROM "http_server_request_duration_count" '
            "GROUP BY x_axis_1 ORDER BY y_axis_1 DESC",
            "Requests",
            ptype="bar",
            x=0,
            y=18,
            w=12,
            h=9,
            x_column="http_response_status_code",
            x_label="Status Code",
        ),
        panel(
            7,
            "Client Errors (4xx)",
            "Separate from 5xx on purpose: a 404 storm is a "
        ...
        panel(
            6,
            "Slowest Routes",
            "By total time, so a slow rare route still shows",
            "http_server_request_duration_sum",
...
health["tabs"][0]["panels"] += [
    panel(
        8,
        "Process CPU",
        "Utilisation of the worker process, 0-1 per core",
        "process_runtime_cpython_cpu_utilization",
        ts("max(value)", "process_runtime_cpython_cpu_utilization"),
        "Utilisation",
        x=0,
        y=36,
        w=8,
    ...
    panel(
        ...
        ts("max(value)", "process_runtime_cpython_memory"),
        "Bytes",
        x=8,
        y=36,
        w=8,
    ...
    panel(
        10,
        "Thread Count",
        "A climbing thread count under steady traffic is a leak",
        "process_runtime_cpython_thread_count",
        ts("max(value)", "process_runtime_cpython_thread_count"),
        "Threads",
        x=16,
        y=36,
        w=8,
...
perf = dashboard(
    ...
    [
        panel(
            ...
            "Total seconds spent serving recommendations",
            "betterbundle_recommendation_duration_sum",
            ts("sum(value)", "betterbundle_recommendation_duration_sum"),
            "Seconds",
            x=0,
            y=0,
            w=12,
        ...
        panel(
            ...
            "Served requests, from the histogram count",
            "betterbundle_recommendation_duration_count",
            ts("sum(value)", "betterbundle_recommendation_duration_count"),
            "Requests",
            x=12,
            y=0,
            w=12,
        ...
        panel(
            3,
            "Cache Hits",
            "Lookups served from Redis",
            "betterbundle_cache_hits",
            ts("sum(value)", "betterbundle_cache_hits"),
            "Hits",
            x=0,
            y=9,
            w=8,
        ...
        panel(
            ...
            ts("sum(value)", "betterbundle_cache_misses"),
            "Misses",
            x=8,
            y=9,
            w=8,
...
kpi = dashboard(
    ...
    [
        panel(
            ...
            "Individual recommendations returned",
            "betterbundle_recommendations_served",
            ts("sum(value)", "betterbundle_recommendations_served"),
            "Recommendations",
            x=0,
            y=0,
            w=24,
        ...
        panel(
            2,
            "By Surface",
            "Which storefront surface asked",
            "betterbundle_recommendations_served",
        ...
        panel(
            3,
            "By Source",
            "Edges, fallback or cache -- how the answer was found",
            "betterbundle_recommendations_served",
...
logs = dashboard(
    ...
    [
        ...
        panel(
            ...
            "Sampling-corrected: each record counts "
            ...
            'FROM "default" GROUP BY x_axis_1 ORDER BY x_axis_1',
            "Log lines (est.)",
            x=0,
            y=0,
            w=24,
        ...
        panel(
            3,
            "Volume by Severity",
            "Sampling-corrected. Without the correction "
...
llm = dashboard(
    ...
    [
        panel(
            1,
            "Spend Over Time",
            "USD, summed per time bucket",
            f"{NS}_gen_ai_cost",
            ts("sum(value)", f"{NS}_gen_ai_cost"),
            "USD",
            x=0,
            y=0,
            w=24,
        ...
        panel(
            2,
            "Spend by Model",
            "Which model the money is going to",
            f"{NS}_gen_ai_cost",
            ...
            w=12,
            h=9,
            x_column="gen_ai_request_model",
            x_label="Model",
        ),
        panel(
            3,
            "Tokens by Type",
            "Input vs output -- they are priced differently, "
            ...
            w=12,
            h=9,
            x_column="gen_ai_token_type",
            x_label="Token Type",
        ),
        panel(
            4,
            "Token Volume Over Time",
            "Total tokens per time bucket",
            "gen_ai_client_token_usage_sum",
            ts("sum(value)", "gen_ai_client_token_usage_sum"),
            "Tokens",
            x=0,
            y=18,
            w=12,
        ...
        # Filtered to input tokens, which is what makes this a call count.
        #
        # `gen_ai_token_usage.record()` is called TWICE per completion — once
        # for input tokens and once for output — so the histogram's _count is
        # exactly twice the number of calls. Unfiltered, this panel reported
        # 192 for a period with 96 completions.
        #
        # Note this counts SUCCESSFUL completions only: usage is recorded after
        # the response returns, so retries and failures are invisible here. For
        # the same period traces showed 497 HTTP requests to Gemini, 361 of
        # them errors — see the External Dependencies dashboard for that view.
        panel(
            5,
            "LLM Calls (successful)",
            "Completions that returned usage. Retries "
            "and failures are NOT counted — see External Dependencies for those",
            "gen_ai_client_token_usage_count",
            "SELECT histogram(_timestamp) as x_axis_1, sum(value) as y_axis_1 "
            'FROM "gen_ai_client_token_usage_count" '
            "WHERE gen_ai_token_type = 'input' GROUP BY x_axis_1 ORDER BY x_axis_1",
            "Calls",
            x=12,
            y=18,
            w=12,
        ...
        # An unpriced model is the failure mode this dashboard cannot show on
        # its own: cost_usd() returns 0 for a model missing from
        # llm_pricing.PRICES, so "Spend Over Time" draws a clean flat zero and
        # reads as "we spent nothing" rather than "we do not know". That is
        # exactly what happened — AI_CHAT_MODEL was moved to
        # gemini-3.1-flash-lite, the price table still only had the 2.x models,
        # and the dashboard reported $0.00 across 96 real completions.
        #
        # The warning already existed in llm_provider._record_usage; nothing
        # surfaced it. This panel is that warning, on the dashboard whose
        # numbers it invalidates.
        panel(
            6,
            "⚠ Unpriced Models",
            "Non-zero means the spend figures above are "
            "too low: these models were called but are missing from "
            "llm_pricing.PRICES, so their cost was recorded as 0",
            "default",
            "SELECT histogram(_timestamp) as x_axis_1, count(*) as y_axis_1 "
            "FROM \"default\" WHERE body LIKE '%No price entry for model%' "
            "GROUP BY x_axis_1 ORDER BY x_axis_1",
            "Warnings",
            x=0,
            y=27,
...
db = dashboard(
    ...
    [
        panel(
            ...
            "Mean statement duration across all engines",
            "default",
        ...
        panel(
            2,
            "Query Throughput",
            "Statements executed per time bucket",
            "default",
            "SELECT histogram(_timestamp) as x_axis_1, count(*) as y_axis_1 "
            'FROM "default" WHERE db_system IS NOT NULL '
            "GROUP BY x_axis_1 ORDER BY x_axis_1",
        ...
        panel(
            3,
            "Slowest Operations",
            "By average duration -- a rare slow query still shows",
            "default",
            ...
            h=9,
            stream_type="traces",
            x_column="operation_name",
            x_label="Operation",
        ),
        panel(
            4,
            "Statements by Engine",
            "Postgres vs Redis share of the traffic",
            "default",
        ...
        panel(
            ...
            "The saturation signal: a pool pinned at its "
            "ceiling is why requests queue",
            "db_client_connections_usage",
            ts("max(value)", "db_client_connections_usage"),
            "Connections",
            x=0,
            y=18,
            w=12,
...
theme = dashboard(
    ...
    [
        panel(
            1,
            "Themes Where The Button Probe Failed",
            "These are the themes to fix "
            ...
            h=9,
            stream_type="traces",
            x_column="theme_adapt_theme_name",
            x_label="Theme",
        ),
        panel(
            2,
            "Which Selector Tier Answered",
            "platform / structural / substring -- "
            ...
            h=9,
            stream_type="traces",
            x_column="theme_adapt_button",
            x_label="Selector",
        ),
        panel(
            3,
            "Button Fill Style",
            "outline-inverted is the branch where we change "
            ...
            h=9,
            stream_type="traces",
            x_column="theme_adapt_button_fill",
            x_label="Fill",
        ),
        panel(
            4,
            "Price Probe Outcome",
            "The probe most likely to miss, since it has no "
            ...
            h=9,
            stream_type="traces",
            x_column="theme_adapt_price",
            x_label="Selector",
        ),
        panel(
            5,
            "Adaptation Switched Off",
            "Merchants driving colours from block " "settings instead",
            "default",
...
reconcilers = dashboard(
    ...
    [
        panel(
            1,
            "Runs by Reconciler",
            "One series per loop. A series that "
        ...
        panel(
            2,
            "Failed Runs",
            "status=error only. Empty is the healthy state, "
            ...
            f"GROUP BY x_axis_1 ORDER BY y_axis_1 DESC",
            "Failures",
            ptype="bar",
            x=12,
            y=0,
        ...
        panel(
            3,
            "Run Rate Over Time",
            "Liveness as a time series: each loop should "
            "show a steady cadence at its own interval",
            f"{NS}_reconciler_runs_total",
            ts("sum(value)", f"{NS}_reconciler_runs_total"),
            "Runs",
            x=0,
            y=9,
            w=24,
        ...
        panel(
            ...
            "Total seconds per bucket. A sweep growing "
            "without the work growing means it is falling behind",
            f"{NS}_reconciler_duration_sum",
            ts("sum(value)", f"{NS}_reconciler_duration_sum"),
            "Seconds",
            x=0,
            y=18,
            w=12,
        ...
        panel(
            5,
            "Slowest Reconcilers",
            "By total time, so a slow rare sweep still shows",
            f"{NS}_reconciler_duration_sum",
            f"SELECT reconciler as x_axis_1, sum(value) as y_axis_1 "
        ...
        panel(
            7,
            "Orders Recovered by Attribution",
            "Stamped orders that were never "
            "attributed — each one is revenue that would not have been billed",
            f"{NS}_reconciler_attribution_missed_orders",
            ts("sum(value)", f"{NS}_reconciler_attribution_missed_orders"),
            "Orders",
            x=12,
            y=27,
            w=12,
        ...
        panel(
            8,
            "Shops Reactivated on Cycle Rollover",
            "Suspended shops whose Shopify " "usage cycle renewed",
            f"{NS}_reconciler_rollover_reactivated_shops",
            ts("sum(value)", f"{NS}_reconciler_rollover_reactivated_shops"),
            "Shops",
            x=0,
            y=36,
            w=12,
        ...
        panel(
            9,
            "Commissions Drained on Reactivation",
            "Backlogged commissions billed " "once a shop came back",
        ...
        panel(
            10,
            "Reconciler Errors in Logs",
            "The traceback behind a failed run",
            "default",
            "SELECT histogram(_timestamp) as x_axis_1, count(*) as y_axis_1 "
...
# Every API this backend calls, and how often it fails.
#
# Built from TRACES, not metrics, and that is the point: the httpx
# instrumentation emits spans but no metrics, so there is no
# `http_client_request_duration` stream for python-worker to chart. The only
# record of an outbound call is its span. (The `http_client_duration_*` metric
# streams that do exist belong to remix-app's Node instrumentation alone.)
#
# This dashboard exists because nothing charted outbound calls at all, which
# is how a 73% error rate against generativelanguage.googleapis.com — 361
# failures in 497 requests — went unnoticed while the LLM cost dashboard
# reported healthy-looking numbers beside it.
#
# `span_kind = 3` is CLIENT. `server_address IS NOT NULL` keeps database and
# Redis spans out: those are CLIENT spans too, and they would otherwise swamp
# a panel meant to be about third-party APIs.
CLIENT_SPANS = "span_kind = 3 AND server_address IS NOT NULL"

deps = dashboard(
    "External Dependencies",
    "Outbound API calls: who we call, how often, and how often it fails",
    [
        panel(
            1,
            "Calls by Host",
            "Every third-party API this backend depends on",
            "default",
            f"SELECT server_address as x_axis_1, count(*) as y_axis_1 "
            f'FROM "default" WHERE {CLIENT_SPANS} '
            f"GROUP BY x_axis_1 ORDER BY y_axis_1 DESC",
            "Calls",
            ptype="bar",
            x=0,
            ...
            h=9,
            stream_type="traces",
            x_column="server_address",
            x_label="Host",
        ),
        panel(
            2,
            "Failures by Host",
            "The panel this dashboard was built for. Read it "
            "next to 'Calls by Host' — a big number here is only alarming "
            "relative to the volume beside it",
            "default",
            f"SELECT server_address as x_axis_1, count(*) as y_axis_1 "
            f"FROM \"default\" WHERE {CLIENT_SPANS} AND span_status = 'ERROR' "
            f"GROUP BY x_axis_1 ORDER BY y_axis_1 DESC",
            "Failures",
            ptype="bar",
            x=12,
        ...
        panel(
            3,
            "Outbound Call Rate",
            "Total third-party calls over time",
            "default",
            f"SELECT histogram(_timestamp) as x_axis_1, count(*) as y_axis_1 "
            f'FROM "default" WHERE {CLIENT_SPANS} '
            f"GROUP BY x_axis_1 ORDER BY x_axis_1",
            "Calls",
            x=0,
            y=9,
            w=12,
        ...
        panel(
            4,
            "Outbound Failures Over Time",
            "A step change here dates an upstream outage",
            "default",
            f"SELECT histogram(_timestamp) as x_axis_1, count(*) as y_axis_1 "
            f"FROM \"default\" WHERE {CLIENT_SPANS} AND span_status = 'ERROR' "
            f"GROUP BY x_axis_1 ORDER BY x_axis_1",
        ...
        panel(
            ...
            "Mean span duration per host, microseconds. A "
            "dependency that slows down shows here before it shows as our latency",
            "default",
            f"SELECT server_address as x_axis_1, avg(duration) as y_axis_1 "
            f'FROM "default" WHERE {CLIENT_SPANS} '
            f"GROUP BY x_axis_1 ORDER BY y_axis_1 DESC",
            "Microseconds",
            ptype="table",
        ...
        # Broken out from the hosts above because these are the two that cost
        # money per call, and because the LLM Cost dashboard cannot show them:
        # it is built on usage metrics that are only recorded after a call
        # SUCCEEDS, so a failing model is invisible there by construction.
        panel(
            6,
            "LLM API Calls by Outcome",
            "Real HTTP requests to the model API, "
            "including retries and failures — not the same number as 'LLM Calls "
            "(successful)' on the LLM Cost dashboard, and the gap is the point",
            "default",
            "SELECT span_status as x_axis_1, count(*) as y_axis_1 "
            'FROM "default" WHERE span_kind = 3 '
            "AND server_address LIKE '%generativelanguage%' "
            "GROUP BY x_axis_1 ORDER BY y_axis_1 DESC",
            "Requests",
            ptype="donut",
            x=12,
            ...
            h=9,
            stream_type="traces",
            x_column="span_status",
            x_label="Outcome",
        ),
        panel(
            7,
            "Embedding Calls by Outcome",
            "Vertex AI embedding requests. These are "
            "gRPC, not HTTP, so they appear only because embedding.py records "
            "its own spans — generic instrumentation does not see them",
            "default",
            "SELECT span_status as x_axis_1, count(*) as y_axis_1 "
            "FROM \"default\" WHERE operation_name LIKE 'gen_ai.embed%' "
            "GROUP BY x_axis_1 ORDER BY y_axis_1 DESC",
            "Requests",
            ptype="donut",
            x=0,
        ...
        panel(
            8,
            "Shopify API Calls",
            "Admin GraphQL and CDN traffic over time",
            "default",
            "SELECT histogram(_timestamp) as x_axis_1, count(*) as y_axis_1 "
            'FROM "default" WHERE span_kind = 3 '
            "AND server_address LIKE '%shopify%' "
            "GROUP BY x_axis_1 ORDER BY x_axis_1",
            "Calls",
            x=12,
            y=27,
