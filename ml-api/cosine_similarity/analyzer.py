...
class BundleAnalyzer:
    ...
    def analyze_co_purchase_patterns(
        ...
        """
        try:
            # Create product ID mapping - ensure all IDs are strings
            ...
            for order in orders:
                ...
                for item in order.line_items:
                    ...
                    # Ensure product_id is a string and exists in our product set
                    if product_id and str(product_id) in product_ids:
                        order_products.add(str(product_id))
                        product_frequencies[str(product_id)] += 1
            ...
            for (
                ...
                if co_purchase_count >= 1:  # Minimum threshold
                    ...
                    # Apply thresholds
                    if (
                        support >= self.bundle_config.min_support
                        and confidence >= self.bundle_config.min_confidence
                        and lift >= self.bundle_config.min_lift
                    ):
    ...
    def get_similar_products(
        self, target_product_id: str, products: List[ProductData], top_k: int = 10
    ) -> List[Dict[str, Any]]:
        """
